...
#k-fold
k_set = [3,5,10,20,50,100]
#classification threshold
#threshold_list = np.arange(0.1,0.85,0.05)
threshold = 0.5
...
for k in k_set:
    accuracy_validation = np.zeros(k)
    ...
    for kfold_idx in range(0,k): 
        ...
        accuracy_train[k_fold_idx] = f1score_train
        accuracy_validation[k_fold_idx] = f1score_validation
        
        k_fold_idx += 1
        
    print("k=%d, F1 for training=%f, F1 for valid=%f" %(k,np.average(accuracy_train),np.average(accuracy_validation)))