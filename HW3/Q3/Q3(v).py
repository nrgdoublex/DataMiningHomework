...
DT_accuracy_validation = np.zeros(k_fold)
DT_accuracy_train = np.zeros(k_fold)
...
for idx, algorithm in enumerate(algorithm_set):
    ...
    for part in range(k_fold):
        ...
        if algorithm == 'logistic':
            ...
            DT_accuracy_validation[part] = f1score_validation
...
data_linear = sorted(DT_accuracy_validation)
...
plt.savefig("Q3(v).png")

(t_score,p_value) = one_tailed_ttest(logistic_accuracy_validation, DT_accuracy_validation)
print "The T-statistic of LR VS DT = {0}".format(t_score)
print "The p-value of LR VS DT = {0}".format(p_value)
