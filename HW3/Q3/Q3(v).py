...
DT_accuracy_validation = np.zeros(k)
DT_accuracy_train = np.zeros(k)
...
for algorithm in algorithm_set:
    ...
    for kfold_idx in range(0,k): 
        ...
        if algorithm == 'logistic':
            ...
            DT_accuracy_validation[k_fold_idx] = f1score_validation 
...
pl.plot(data_linear,fit_linear,'-v',label='Decision Tree')
