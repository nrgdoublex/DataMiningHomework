...
for reg in reg_type:
    ...
    dt_index = np.random.choice(len(target_train), size=train_sizes, replace=True)
    data_train_1 = data_train[dt_index,]
    target_train_1 = target_train[dt_index]

    # Describe classifier and regularization type
    logr = LogisticRegression(penalty=reg,C=regularization_strength,fit_intercept=False,class_weight="balanced",solver="liblinear")
    # Train model
    ...
    f1score_train += f1_score(target_train_1, y_pred_train_1)
    
    print(np.count_nonzero([np.abs(i) > 0.001 for i in logr.coef_]))
