...
count = 0
with open('Q1(e)_ans.txt','w') as f:
    for _, county in df.iterrows():
        data = county[['State','County','Number2013','Percent2013','Number2012','Percent2012']]
        if any(pd.isnull(data)): continue
        popu2013 = float(data['Number2013'])
        per2013 = float(data['Percent2013'])
        popu2012 = float(data['Number2012'])
        per2012 = float(data['Percent2012'])
        
        t_score = ttest(popu2013, per2013, popu2012, per2012, diff)
