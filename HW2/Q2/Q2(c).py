...
def play_arm(round,arm_idx,proba,pos_case):
    global revenue
    global cost
    global buyrate
    global earn_money
    ...
    #renew best arm
    return np.argmax(pos_case[:,round])
...
#randomized experiment - to initialize best arm
round_idx = 0
...
while np.max(pos_case[:,round_idx]) == 0:
    round_idx += 1
    arm_idx = np.random.randint(0,len(pos_case)-1)
    play_arm(round_idx,arm_idx, per_2013[county_idx[arm_idx]], pos_case)
        
#print pos_case[:,round_idx]
#print np.argmax(pos_case[:,round_idx])
#-----------------------------------------epsilon algorithm-----------------------------------------
#initialize best arm
best_arm_idx = np.argmax(pos_case[:,round_idx])
...
while round_idx < total_invi:
    ...
    if rv >= epsilon:
        #play best arm
        best_arm_idx = play_arm(round_idx,best_arm_idx, per_2013[county_idx[best_arm_idx]], pos_case)
    else:
        #play random arms
        arm_idx = np.random.randint(0,len(pos_case)-1)
        best_arm_idx = play_arm(round_idx,arm_idx, per_2013[county_idx[arm_idx]], pos_case)
