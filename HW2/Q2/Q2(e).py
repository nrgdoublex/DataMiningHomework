...
while round_idx < total_invi:
    ...
    theta = np.zeros(len(county_idx))
    for i in range(0,len(county_idx)):
        theta[i] = np.random.beta(pos_case[:,round_idx][i]+1,total_case[:,round_idx][i]-pos_case[:,round_idx][i]+1)
    best_arm_idx = np.argmax(theta)
    play_arm(round_idx,best_arm_idx, per_2013[county_idx[best_arm_idx]], pos_case)
