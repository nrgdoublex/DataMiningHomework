...
def ucb(num_invi, round_idx, num_pull, num_arm):
    ucb = np.zeros(num_arm)
    for i in range(num_arm):
        if num_invi[i][round_idx] == 0 or round_idx == 1:
            ucb[i] = np.inf
        else:
            ucb[i] = float(num_invi[i][round_idx])/num_pull[i] + np.sqrt(2*np.log(round_idx)/num_pull[i])
...
while round_idx < total_invitations:
    ...
    # choose best arm by UCB1
    best_arm = ucb(num_invitations_county,round_idx-1, num_pull_arm, df.shape[0])
